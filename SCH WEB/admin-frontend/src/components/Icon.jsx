...
const Icon = ({ name, size = 20, className = '', color = 'currentColor' }) => {
  const icons = {
    ...
    userCog: (
      <svg viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2">
        ...
        <circle cx="19" cy="14" r="2.5" /><path d="M19 10.5v1M19 16.5v1M22 14h-1M17 14h-1" />
      </svg>
    ),
    eye: (
      <svg viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2">
  ...
  return (
    ...
    >
      {React.cloneElement(icons[name] || icons.alertCircle, { width: size, height: size })}
    </span>
  );
};
